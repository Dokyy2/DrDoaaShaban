...
function fileToBase64(file) {
    return new Promise(resolve => {
        if (!file) return resolve({ attachmentName: "", attachmentBase64: "", attachmentType: "" });
        ...
        reader.onerror = () => resolve({ attachmentName: file.name, attachmentBase64: "", attachmentType: file.type || "" });
...
function isSupabaseConfigured() {
    return typeof window.CLINIC_SUPABASE !== "undefined" &&
        window.CLINIC_SUPABASE.url && window.CLINIC_SUPABASE.anonKey;
}

async function sendBooking(payload) {
    if (!isSupabaseConfigured()) {
        const res = await fetch(SCRIPT_URL, { method: "POST", body: JSON.stringify(payload) });
        if (!res.ok) throw new Error("تعذر الاتصال بخدمة الحجز");
        return res.json();
    }

    const { url, anonKey } = window.CLINIC_SUPABASE;
    ...
    if (!res.ok) throw new Error(response.message || "تعذر إرسال طلب الحجز");
    return response;
}

function showSubmissionError(message) {
    document.getElementById("bookingArea").innerHTML = `
        <div class="result-card submission-error">
            <div class="result-icon"><i class="fa-solid fa-circle-xmark"></i></div>
            <h2>${currentLang === "ar" ? "لم يتم تسجيل الحجز" : "Booking was not registered"}</h2>
            <p>${message || (currentLang === "ar" ? "حدثت مشكلة في الاتصال. من فضلكِ أعيدي المحاولة أو تواصلي مع العيادة." : "A connection problem occurred. Please try again or contact the clinic.")}</p>
            <button class="inline-back-btn" onclick="renderBookingForm()">${currentLang === "ar" ? "إعادة المحاولة" : "Try again"}</button>
        </div>`;
    document.getElementById("bookingArea").scrollIntoView({ behavior: "smooth", block: "center" });
}
...
async function submitBooking() {
    ...
    const payload = {
        ...
        // "Join the clinic family" is a contact-list registration, not an appointment.
        bookingDate: currentBookingType === "contacts" ? "" : valid.date,
    ...
    try {
        ...
        if (response.status === "duplicated") {
            ...
            showDuplicated(oldData, String(response.bookingId || finalId), response.message || (currentLang === "ar" ? "يوجد حجز مسبق بهذا الرقم." : "There is already a booking for this number."), hasAttachment, message);
            return;
        }
...
async function submitBirth() {
    ...
    try {
        ...
        if (response.status === "duplicated") {
            ...
            showDuplicated(oldData, String(response.bookingId || finalId), response.message || (currentLang === "ar" ? "يوجد حجز مسبق بهذا الرقم." : "There is already a booking for this number."), false, message);
            return;
        }
